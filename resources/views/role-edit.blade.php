        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="unpaid-tab" data-bs-toggle="tab" data-bs-target="#unpaid" type="button"
                    role="tab" aria-controls="unpaid" aria-selected="true">Edit Role</button>
            </li>
            <a href="{{ route('role-index') }}" class="text-decoration-none"><button class="nav-link">View
                    Roles</button></a>
        </ul>
                    ...
                    <form method="POST" action="{{route('role-update', $role)}}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control" id="Currency-name"
                                placeholder="Category Name" value="{{ old('name',$role->name) }}">
                            @if ($errors->has('name'))
                        ...
                        <button type="submit" class="btn blue_button">Update Role</button>
                    </form>
                </div>
            </div>
        </div>

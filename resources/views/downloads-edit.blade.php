        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="unpaid-tab" data-bs-toggle="tab" data-bs-target="#unpaid" type="button"
                    role="tab" aria-controls="unpaid" aria-selected="true">Edit Downloads</button>
            </li>
                    ...
                    <form method="POST" action="{{ route('download-update', $download) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control" id="Currency-name"
                                placeholder="Title " value="{{ old('title',$download->title) }}">
                            </select>
                            @if ($errors->has('title'))
                        ...
                        <div class="row mb-3">
                            <div class="col-6">
                                ...
                                <select class="form-select" name="type" required aria-label="Default select example">
                                    <option {{ old('type', $download->type) == "Menu" ? 'selected' : '' }} value="Menu">Menu</option>
                                    <option {{ old('type', $download->type) == "Offers" ? 'selected' : '' }} value="Offers">Offers</option>
                                </select>
                                @if ($errors->has('type'))
                            ...
                            <div class="col-6">
                                ...
                                <select class="form-select" name="category" required aria-label="Default select example">
                                    <option {{ old('category', $download->category) == "Drinks" ? 'selected' : '' }} value="Drinks">Drinks</option>
                                    <option {{ old('category', $download->category) == "Foods" ? 'selected' : '' }} value="Foods">Foods</option>
                                </select>
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                ...
                                <input type="test" name="detail" class="form-control" id="Currency-name"
                                    placeholder=" Details " value="{{ old('detail',$download->detail) }}">
                        ...
                        <button type="submit" class="btn blue_button">Update Download</button>
                    </form>
                </div>
            </div>
        </div>

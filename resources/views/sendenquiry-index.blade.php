        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="paid-tab" data-bs-toggle="tab" data-bs-target="#paid" type="button"
                    role="tab" aria-controls="paid" aria-selected="false">Enquiry</button>
            </li>
        </ul>
                ...
                <div class="table-responsive p-3">
                    <table class="table" style="width: 100%">
                        ...
                        <tbody>
                            ...
                            @foreach ($sendenquiries as $key => $sendenquiry)
                                <tr class="">
                                    <td scope="row">{{ $key + 1 }}</td>
                                    <td> {{ $sendenquiry->name }}</td>
                                    <td>{{ $sendenquiry->email }} </td>
                                    <td>{{ $sendenquiry->contact }} </td>
                                    <td>{{ $sendenquiry->course }} </td>
                                    <td>{{ $sendenquiry->college }} </td>
                                    <td>{{ $sendenquiry->message }} </td>
                    ...
                    <div class="col-7 d-flex justify-content-end">
                        <div class="d-flex p-0 m-0">
                            <a class="text-decoration-none"
                                href="{{ $sendenquiries->withQueryString()->previousPageUrl() }}">
                            ...
                            <div
                                ...
                                {{ $sendenquiries->currentPage() }}

                            </div>


                            <a class="text-decoration-none" href="{{ $sendenquiries->withQueryString()->nextPageUrl() }}">

        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="unpaid-tab" data-bs-toggle="tab" data-bs-target="#unpaid" type="button"
                    role="tab" aria-controls="unpaid" aria-selected="true">Update Payment Method</button>
            </li>
        ...
        <div class="tab-content py-2 navsTabsShadow" id="myTabContent">
            ...
            <div class="tab-pane fade show  active" id="unpaid" role="tabpanel" aria-labelledby="unpaid-tab">
                <div class="container-fluid p-3">
                    <form method="POST" action="{{ route('paymentmethod-update', $paymentmethod) }}"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="col-6">
                            <label for="recipient-name" class="col-form-label">Title:</label>
                            <input type="text" name="title" class="form-control" id="Currency-title"
                                placeholder="Payment title" value="{{ old('title', $paymentmethod->title) }}">
                            @if ($errors->has('title'))
                        ...
                        <div class="col-6">
                            ...
                            <input type="file" id="logo" name="logo" class="form-control"
                                placeholder="Upload logo" value="{{ old('logo', $paymentmethod->logo) }}">
                            @if ($errors->has('logo'))
                        ...
                        <div class="col-6">
                            ...
                            <input type="file" id="image" name="image" class="form-control"
                                placeholder="Upload image" value="{{ old('image', $paymentmethod->image) }}">
                            @if ($errors->has('image'))
                        ...
                        <br>
                        <button type="submit" class="btn blue_button">Update Payment Method</button>
                    </form>
                </div>
            </div>
        </div>

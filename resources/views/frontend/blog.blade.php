@extends('layouts.frontend-layout.navfoot')
@section('content')
    <!-- Blog Section-->
    <h1 class="text-center pt-10 text-2xl font-bold">Our Blogs</h1>

    <div class="card w-10/12 m-auto rounded  h-auto flex flex-wrap justify-around  mt-10  gap-10  ">
        @foreach ($blogs as $blog)
            <div class="w-80 bg-white rounded-lg shadow-md overflow-hidden relative" style="height: 450px;">
                <!-- Image -->
                <img src="{{ asset('images/photos/' . $blog->image) }}" alt="Person" class="w-full h-52 object-fit">

                <!-- Content -->
                <div class="p-4 text-sm h-atuo">
                    <a href="{{ route('blogdesc', $blog) }}">
                        <h2 class="text-xl font-semibold cursor-pointer">{{ $blog->title }}</h2>
                    </a>

                    <p class="text-sm pt-2">{!! $blog->description !!} </p>


                    <div class="mt-10 absolute bottom-0">
                        <hr class="">
                        <p class="pt-2 pb-2">{{ date('d-m-Y', strtotime($blog->created_at)) }}</p>
                    </div>
                </div>
            </div>
        @endforeach



        {{-- <div class="w-80  bg-white rounded-lg shadow-md overflow-hidden relative " style="height: 450px;">
            <!-- Image -->
            <img src="https://www.kitwosd.com/storage/trainings/1675519354Learn.png" alt="Person"
                class="w-full h-52 object-fit">

            <!-- Content -->
            <div class=" p-4 text-sm h-atuo">
                <a href="{{ route('blogdesc') }}">
                    <h2 class="text-xl font-semibold">Top Digital Marketing Skills: Digital Marketing
                        Career and Strategies</h2>
                </a>

                <p class="text-sm pt-2">The Top Digital Marketing Skills You Need to Succeed: Digital Marketing Career
                    and Strategies Words can turn stones into treasures, while silence leaves even gold </p>
                <div class="mt-10 absolute bottom-0">
                    <hr class="mt-3">
                    <p class="pt-2 pb-2">July 3,2023</p>
                </div>
            </div>
        </div>


        <div class="w-80  bg-white rounded-lg shadow-md overflow-hidden relative " style="height: 450px;">
            <!-- Image -->
            <img src="data:image/png;base64,"
                alt="Person" class="w-full h-52 object-fit">

            <!-- Content -->
            <div class=" p-4 text-sm h-atuo">
                <a href="{{ route('blogdesc') }}">
                    <h2 class="text-xl font-semibold">Top Digital Marketing Skills: Digital Marketing
                        Career and Strategies</h2>
                </a>

                <p class="text-sm pt-2">The Top Digital Marketing Skills You Need to Succeed: Digital Marketing Career
                    and Strategies Words can turn stones into treasures, while silence leaves even gold </p>
                <div class="mt-10 absolute bottom-0">
                    <hr class="mt-3">
                    <p class="pt-2 pb-2">July 3,2023</p>
                </div>
            </div>
        </div> --}}
    </div>
@endsection

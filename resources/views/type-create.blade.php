        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="unpaid-tab" data-bs-toggle="tab" data-bs-target="#unpaid" type="button"
                    role="tab" aria-controls="unpaid" aria-selected="true">Add Media Type</button>
            </li>
            ...
            <div class="tab-pane fade show  active" id="unpaid" role="tabpanel" aria-labelledby="unpaid-tab">
                <div class="container-fluid p-3">
                    <form method="POST" action="/type-store">
                        @csrf
                        <div class="mb-3">
                            <label for="recipient-name" class="col-form-label">Media Type Name:</label>
                            <input type="text" name="name" class="form-control" id="Currency-name"
                                placeholder="Media Type Name" value="{{ old('name') }}">
                            @if ($errors->has('name'))
                        ...
                        <button type="submit" class="btn blue_button">Add Media Type</button>
                    </form>
                </div>
            </div>
        </div>
